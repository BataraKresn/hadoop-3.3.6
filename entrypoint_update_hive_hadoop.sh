...
# Function to set JAVA_HOME based on input
set_java_home() {
  case $1 in
    java8)
...
# Default to Java 11 unless specified otherwise
set_java_home ${JAVA_VERSION:-java11}
...
# This will transform hadoop.env format into iterable data format that can be added to addProperty
function configure() {
    local path=$1
    local module=$2
    local envPrefix=$3
...
# If we use multihome network, some additional config will be added into xml config files
